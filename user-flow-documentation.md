...
| **Returning identity** | Email pre-filled from prior session. `Not you?` link. All tiles remain visible. Subtitle greets with "Welcome back" (was misleadingly "Enter your email to get started" prior to W3.7.6). |
| **Returning · passkey available** | Origin has a saved passkey. Site invokes WebAuthn directly via `navigator.credentials.get()`. Layout mirrors the App Face ID / Touch ID pattern: identity chip (`shopper@ace.com`) above a primary **"Sign in with passkey"** CTA carrying a generic passkey glyph. Web-generic "passkey" label (not "Face ID") because the site doesn't know which authenticator will fire — could be Face ID, Touch ID, Windows Hello, YubiKey, or phone-as-authenticator. Password + one-time code fallback tiles remain visible below the divider; the Face ID tile is hidden (redundant with the hero CTA). |

> Design note (W3.7.6): the initial `passkey-available` design (W3.7.5) rendered a dashed-border "browser autofill suggestion" pill inline in the page — an attempt to mock how iOS Safari / Chrome Mobile surface passkey suggestions above the keyboard. That approach was replaced because (a) the pill competed visually with the primary Continue button and the tiles below, and (b) mocking OS chrome as an inline page element caused more confusion than it clarified. The site genuinely can invoke WebAuthn directly (Google, Chase, GitHub, PayPal all do this), so a proper site-owned primary CTA is both accurate and clearer. The current design also aligns Mobile Web / passkey-available visually with App / Returning · Face ID and App / Returning · Touch ID — one consistent "we recognize you, primary path is passkey" pattern across all three surfaces.
